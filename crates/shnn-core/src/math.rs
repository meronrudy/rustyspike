...
// Bring trait-based math methods (exp, ln, sqrt, powi, sin, cos, abs) into scope
use shnn_math::MathExt;
...
pub mod fast {
    ...
    /// Fast square root using Newton-Raphson method (no bit hacks for no_std safety)
    pub fn sqrt_approx(x: f32) -> f32 {
        if x <= 0.0 {
            return 0.0;
        }
        // Robust initial guess without relying on from_bits/to_bits
        let mut y = if x > 1.0 { x * 0.5 } else { 1.0 };
    ...
    /// Fast inverse square root (uses sqrt_approx to avoid bit-level hacks)
...
pub mod activation {
    ...
    pub fn sigmoid_approx(x: f32) -> f32 {
        1.0 / (1.0 + super::fast::exp_approx(-x))
    }
    ...
    pub fn tanh_approx(x: f32) -> f32 {
        let exp_2x = super::fast::exp_approx(2.0 * x);
        (exp_2x - 1.0) / (exp_2x + 1.0)
    }
...
mod tests {
    ...
    fn test_find_peaks() {
        ...
        assert!(peaks.iter().any(|&i| i == 1)); // Value 3.0
        assert!(peaks.iter().any(|&i| i == 3)); // Value 5.0
        assert!(peaks.iter().any(|&i| i == 5)); // Value 4.0
    }
}