...
mod tests {
    ...
    fn test_sensor_preprocessor() {
        ...
        preprocessor.set_channel_params(0, 2.0, 0.5);
        
        let processed = preprocessor.process_sample(0, 1.0);
        assert_eq!(processed, 2.5); // 1.0 * 2.0 + 0.5
        
        // Test filtering
        preprocessor.set_filter_coeff(0, 0.5);
        preprocessor.reset();
        
        let filtered1 = preprocessor.process_sample(0, 1.0);
        let filtered2 = preprocessor.process_sample(0, 1.0);
        
        // Second sample should be closer to target due to filtering
    ...
    fn test_multi_modal_encoder() {
        ...
        // Test that all encoders can be created
        assert_eq!(encoder.rate_encoder.encoding_type(), EncodingType::Rate);
        assert_eq!(encoder.temporal_encoder.encoding_type(), EncodingType::Temporal);
        assert_eq!(encoder.population_encoder.encoding_type(), EncodingType::Population);
        
        encoder.reset(); // Should not crash
    }
}
...
pub struct EncodingConfig {
    ...
    pub default_type: EncodingType,
    /// Rate encoding configuration
    pub rate_config: RateEncodingConfig,
    /// Temporal encoding configuration
    pub temporal_config: TemporalEncodingConfig,
    /// Population encoding configuration
    pub population_config: PopulationEncodingConfig,
    /// Delta encoding configuration
    pub delta_config: DeltaEncodingConfig,
}

impl Default for EncodingConfig {
    fn default() -> Self {
        Self {
            default_type: EncodingType::Rate,
            rate_config: RateEncodingConfig::default(),
            temporal_config: TemporalEncodingConfig::default(),
            population_config: PopulationEncodingConfig::default(),
            delta_config: DeltaEncodingConfig::default(),
        }
    }
}

impl EncodingConfig {
    /// Create configuration for rate encoding
    pub fn rate_only() -> Self {
        Self {
            default_type: EncodingType::Rate,
            ..Default::default()
        }
    ...
    /// Create configuration for temporal encoding
    pub fn temporal_only() -> Self {
        Self {
            default_type: EncodingType::Temporal,
            ..Default::default()
        }
    }
    
    /// Create configuration for population encoding
    pub fn population_only() -> Self {
        Self {
            default_type: EncodingType::Population,
            ..Default::default()
        }
    }
}
...
impl MultiModalEncoder {
    ...
    pub fn from_config(config: EncodingConfig) -> Self {
        Self {
            rate_encoder: RateEncoder::new(),
            temporal_encoder: TemporalEncoder::new(),
            population_encoder: PopulationEncoder::new(),
...
mod tests_extra {
    ...
    #[test]
    fn test_sensor_preprocessor_extended() {
        let mut preprocessor = SensorPreprocessor::new(1);
