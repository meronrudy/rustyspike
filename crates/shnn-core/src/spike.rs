...
/// A collection of spikes from a single neuron
#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
pub struct SpikeTrain {
    ...
    /// Optional spike amplitudes (if None, all spikes are binary)
...
impl SpikeTrain {
    ...
    pub fn add_spike(&mut self, timestamp: Time, amplitude: Option<f32>) {
        ...
        if let Some(amp) = amplitude {
            if let Some(ref mut amps) = self.amplitudes {
                amps.push(amp);
            } else {
                // Convert to amplitude-based train
                let mut amps = vec![1.0; self.timestamps.len() - 1]; // Fill previous with 1.0
                amps.push(amp);
                self.amplitudes = Some(amps);
            }
        } else if let Some(ref mut amps) = self.amplitudes {
            amps.push(1.0); // Default amplitude
        }
    }
